...
//get the balance of user from database with session id
        ...
        $this->queryBuilder->executeQuery();
        $_SESSION['errors']['successfulBuy'] = 'You have successfully bought ' . $_POST['quantity'] . ' shares of ' . $stockSymbol . '!';
    ...
    public function sell()
    {
        ...
        $this->queryBuilder->insert('transactions')
            ...
            ->setParameter('created_at', date('Y-m-d H:i:s'));
        $this->queryBuilder->executeQuery();
        $_SESSION['errors']['successfulSell'] = 'You have successfully sold ' . $_POST['quantity'] . ' shares of ' . $stockSymbol . '!';
        return new Redirect('/profile');
    }
}
